...
export default function ConsultModal({
  ...
  const programBlock = programSelect && (
    <div className="rounded-2xl border-2 border-blue-200 bg-blue-50/50 p-4">
      <p className="text-sm font-bold text-slate-900">어떤 방식으로 진행하고 싶으세요?</p>
  ...
  const fundingBlock = programSelect && (
    <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
      <p className="text-sm font-semibold text-slate-800">
        자금 계획 <span className="font-normal text-slate-400">(선택 · 실제로 필요한지는 진단 후 함께 확인해요)</span>
      </p>
      <div className="mt-3 space-y-3">
  ...
  const axConsentBlock = isAx && (
    <div className="rounded-2xl border-2 border-blue-200 bg-blue-50/40 p-4">
      <p className="text-sm font-bold text-slate-900">AX 참여 조건</p>
      <div className="mt-3 space-y-3.5">
        {/* 레퍼런스 참여 동의 — 필수 2건 */}
  ...
  const topicsBlock = topicGroups.length > 0 && (
    <div>
      ...
      <p className="-mt-0.5 mb-2 text-[0.8rem] leading-snug text-slate-400">안 고르셔도 상담에서 상황에 맞게 추천해 드려요.</p>
  ...
  const messageBlock = (
    <div>
      <label htmlFor="consult-message" className={labelClass}>
        {programSelect ? '지금 가장 어려운 점' : '문의 내용'} <span className="font-normal text-slate-400">(선택)</span>
      </label>
      <textarea
        id="consult-message"
        name="message"
        ...
        placeholder={programSelect ? '자금이나 운영에서 가장 어려운 점을 편하게 적어 주세요. 정확하지 않아도 괜찮아요.' : '궁금한 점이나 지금 상황을 편하게 적어 주세요.'}
  ...
  const messageExtraBlock = programSelect && (
    <div>
      <label htmlFor="consult-message-extra" className={labelClass}>
        추가 문의 <span className="font-normal text-slate-400">(선택)</span>
      </label>
      <textarea
  ...
  const reviewBlock = programSelect && reviewRows.length > 0 && (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
      <p className="text-[0.78rem] font-black uppercase tracking-wide text-slate-400">입력하신 내용 한눈에 보기</p>
