...
function ModuleGroupCard({ m, defaultOpen }: { m: (typeof GROWTH_MODULES)[number]; defaultOpen: boolean }) {
  ...
  return (
    <div id={m.id} className="scroll-mt-24 rounded-2xl border border-slate-200 bg-white">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
    ...
    <div>
      ...
      <div className="mt-6 flex justify-center">
        <button
          ...
          className="flex min-h-[56px] w-full items-center justify-center gap-2 rounded-xl bg-slate-900 px-6 text-[1.15rem] font-black text-white transition-transform hover:-translate-y-0.5 sm:w-auto"
        >
      ...
      {allOpen && (
        <div id="all-modules" className="mt-6 scroll-mt-24 rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
          <p className="text-[1.2rem] font-black text-slate-900">지금 진행 가능한 컨설팅 전체</p>
          <p className="mt-2 break-keep text-[1.06rem] leading-relaxed text-slate-500">
            모두 진행해야 하는 것이 아닙니다. 진단 후 지금 필요한 것부터 순서를 잡습니다.
          </p>
          <div className="mt-5 grid gap-5 sm:grid-cols-2">
            {GROWTH_MODULES.map((m) => (
              <div key={m.id}>
                <p className={`text-[1.09rem] font-black ${m.accent}`}>{m.title}</p>
                <ul className="mt-2 space-y-1">
                  {MODULE_MEMBERS[m.group].map((mp) => (
                    <li key={mp.slug}>
                      <Link
                        to={`/business-services/${mp.slug}`}
                        className="flex items-center justify-between gap-2 rounded-lg px-2 py-2 text-[1.06rem] font-semibold text-slate-700 transition-colors hover:bg-slate-50 hover:text-blue-700"
                      >
                        <span className="min-w-0 truncate">{mp.name}</span>
          ...
          <Link
            ...
            className="mt-6 flex min-h-[52px] items-center justify-center gap-2 rounded-xl bg-blue-500 px-5 text-[1.12rem] font-black text-white transition-colors hover:bg-blue-400"
          >
