    ...
    <div id={m.id} className="min-w-0 scroll-mt-24 rounded-2xl border border-slate-200 bg-white">
      ...
      >
        <span className="flex items-center gap-2">
          <span className={`text-[1.29rem] sm:text-[1.521rem] font-black tabular-nums ${m.accent}`}>{m.no}</span>
          <span className="text-[1.45rem] sm:text-[1.716rem] font-black leading-snug text-slate-900">{m.title}</span>
        </span>
          ...
          {MODULE_MEMBERS[m.group].map((mp) => (
            <li key={mp.slug}>
              <Link
                ...
                className="group -mx-0.5 flex items-center justify-between gap-2 rounded-lg px-2.5 py-2.5 text-[1.24rem] sm:text-[1.469rem] font-bold text-slate-700 transition-colors hover:bg-slate-50 hover:text-blue-700 focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-500"
              >
    ...
    <div>
      ...
      <ol className="mt-6 flex flex-wrap gap-2">
        {LIFECYCLE.map((a, i) => (
          <li
            key={a.t}
            className="flex min-w-0 flex-1 basis-full items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 min-[360px]:basis-[calc(50%-0.25rem)] lg:basis-[calc(33.333%-0.34rem)]"
          >
            <span aria-hidden className="text-[1.58rem] sm:text-[1.872rem]">{a.icon}</span>
            <span className="min-w-0">
              <span className="block text-[1.13rem] sm:text-[1.339rem] font-black text-slate-400">STEP {i + 1}</span>
              <span className="block break-keep text-[1.26rem] sm:text-[1.495rem] font-bold leading-tight text-slate-800">{a.t}</span>
            </span>
          </li>
        ))}
      </ol>
      ...
      {/* 네 그룹으로 나눠 보는 것과 별개로, 썸네일·가격이 보이는 전체 목록 페이지로 이동한다 */}
