    ...
    <div>
      <ul className="grid gap-2 sm:grid-cols-2">
        {AX_POLICY_EVIDENCE_2026.map((e) => (
          ...
          >
            <p className={`text-[1.09rem] font-black leading-snug ${dark ? 'text-teal-200' : 'text-blue-700'}`}>{e.title}</p>
            <p className={`mt-1.5 break-keep text-[1.13rem] leading-relaxed ${dark ? 'text-slate-300' : 'text-slate-600'}`}>
              {e.shortDescription}
            </p>
            <p className={`mt-2 break-keep text-[1.01rem] leading-snug ${dark ? 'text-slate-500' : 'text-slate-400'}`}>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-2">
        <Link
          ...
          className={`inline-flex min-h-[46px] items-center gap-1.5 rounded-xl px-4 text-[1.13rem] font-bold transition-colors ${
            dark
      ...
      <p className={`mt-2.5 break-keep text-[1.03rem] leading-relaxed ${dark ? 'text-slate-500' : 'text-slate-500'}`}>
