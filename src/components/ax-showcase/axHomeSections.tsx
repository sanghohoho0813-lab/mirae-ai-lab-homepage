// 홈 재구성용 섹션 모음 — Hero / 공감 / 세 가지 가치 / 5단계 방법론 / 2주 과정 / 결과물 / 선별기준.
...
import {
  ...
  AX_METHOD_STEPS,
  AX_MVP_DEFINITION,
  AX_SELECTION_DECLINE,
  AX_SELECTION_PRIORITY,
  AX_TIMELINE,
  AX_TIMELINE_EXCLUDED,
  AX_TIMELINE_INCLUDED,
  AX_TIMELINE_NOTICE,
} from '../../data/axPackages'

const band = 'px-5 py-11 sm:px-6 sm:py-16'
const wrap = 'mx-auto max-w-5xl'
const h2Dark = 'break-keep text-[1.6rem] font-black leading-tight text-white sm:text-[2.15rem]'
...
export function AxHeroV2({ onShowcase }: { onShowcase: () => void }) {
  return (
    <section className="relative flex min-h-[calc(100svh-53px)] items-center overflow-hidden bg-slate-950">
      ...
      <div className={`relative w-full ${wrap} px-5 pb-20 pt-12 sm:px-6 sm:pb-24 sm:pt-16`}>
        ...
        <div className="mt-7 flex flex-col gap-2.5 sm:max-w-2xl sm:flex-row">
          <button
            type="button"
            onClick={onShowcase}
...
/** SECTION 2 — 고객의 자금문제 공감. 짧게 공감하고 바로 해결 가능성을 제시한다. */
export function AxEmpathyV2() {
  return (
    <section className={`${band} border-t border-white/10 bg-slate-900`}>
      <div className={wrap}>
        <h2 className={h2Dark}>
...
/** SECTION — 최대 2주 진행과정. 포함/미포함 범위를 분명히 구분한다. */
export function AxTimelineSection() {
  return (
    <section id="process" className={`${band} scroll-mt-16 border-t border-slate-200 bg-white`}>
      <div className={wrap}>
        <h2 className={h2Light}>
          오래 기다리게 하지 않습니다.<br className="hidden sm:block" /> 빠르게 보여드리고 함께 완성합니다.
        </h2>
...
/** SECTION — 실제 제공 결과물 */
export function AxDeliverablesSection() {
  return (
    <section id="deliverables" className={`${band} scroll-mt-16 border-t border-slate-200 bg-slate-50`}>
      <div className={wrap}>
        <h2 className={h2Light}>
...
/** SECTION — 고객 선별기준. 월 5개사 문구는 사이트 전체 최대 두 번만 사용한다. */
