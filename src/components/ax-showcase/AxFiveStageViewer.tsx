    ...
    <div className="rounded-3xl border border-white/12 bg-gradient-to-b from-white/[0.07] to-white/[0.03] p-3 shadow-xl shadow-slate-950/40 sm:p-5">
        ...
        {industry.stages.map((s, i) => {
          ...
          return (
            <button
              ...
              aria-selected={on}
              onClick={() => setIdx(i)}
              className={`flex min-h-[44px] items-center justify-center gap-1.5 break-keep rounded-lg px-2 py-1.5 text-[1.1rem] font-bold leading-tight transition-colors sm:text-[1.3rem] ${
                on ? 'bg-teal-400 text-slate-900' : 'bg-white/5 text-slate-400 ring-1 ring-inset ring-white/10 hover:text-white'
              }`}
            >
              <span className={`grid h-5 w-5 shrink-0 place-items-center rounded-full text-[1.1rem] sm:text-[1.3rem] font-black ${on ? 'bg-slate-900/15 text-slate-900' : 'bg-white/10 text-slate-300'}`}>{s.no}</span>
      ...
      <div className="relative mt-3">
        ...
        <button
          ...
          disabled={idx === 0}
          aria-label="이전 단계"
          className="absolute left-1 top-1/2 hidden h-11 w-11 -translate-y-1/2 place-items-center rounded-full border border-white/15 bg-slate-950/70 text-lg sm:text-[1.463rem] text-white backdrop-blur transition-opacity hover:bg-slate-900 disabled:opacity-25 sm:grid"
        >
          ←
        </button>
        <button
          ...
          aria-label="다음 단계"
          className="absolute right-1 top-1/2 hidden h-11 w-11 -translate-y-1/2 place-items-center rounded-full border border-white/15 bg-slate-950/70 text-lg sm:text-[1.463rem] text-white backdrop-blur transition-opacity hover:bg-slate-900 disabled:opacity-25 sm:grid"
        >
          →
        </button>

        {/* 현재 위치 */}
        <span className="pointer-events-none absolute right-3 top-3 rounded-full bg-slate-950/75 px-2.5 py-1 text-[1.1rem] sm:text-[1.3rem] font-black text-teal-200 ring-1 ring-inset ring-white/15">
          {cur.no} / {total}
        </span>
      </div>
      ...
      <div className="mt-3.5">
        <p className="text-[1.1rem] sm:text-[1.3rem] font-black tracking-tight text-teal-300">
          {cur.no}단계 · {cur.label}
        </p>
        <h4 className="mt-1.5 break-keep text-[1.39rem] font-black leading-snug text-white sm:text-[1.755rem]">{cur.title}</h4>
        <p className="mt-2 max-w-3xl break-keep text-[1.2rem] leading-relaxed text-slate-300 sm:text-[1.495rem]">{cur.desc}</p>
        {cur.chips.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-1.5">
            {cur.chips.map((c) => (
              <span key={c} className="rounded-lg bg-white/5 px-2.5 py-1 text-[1.1rem] sm:text-[1.3rem] font-semibold text-slate-300 ring-1 ring-inset ring-white/10">{c}</span>
            ))}
          </div>
        )}
      </div>
      ...
      <div className="mt-3.5 flex items-center gap-2 sm:hidden">
        <button type="button" onClick={() => go(-1)} disabled={idx === 0} className="min-h-[44px] flex-1 rounded-xl border border-white/15 bg-white/5 text-[1.11rem] sm:text-[1.313rem] font-bold text-white disabled:opacity-30">← 이전</button>
        <button type="button" onClick={() => go(1)} disabled={idx === total - 1} className="min-h-[44px] flex-1 rounded-xl border border-white/15 bg-white/5 text-[1.11rem] sm:text-[1.313rem] font-bold text-white disabled:opacity-30">다음 →</button>
      </div>
      ...
      <p className="mt-2.5 text-[1.1rem] sm:text-[1.3rem] text-slate-500">이미지를 누르면 전체화면으로 확인할 수 있습니다.</p>
