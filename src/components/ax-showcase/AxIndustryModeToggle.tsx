    ...
    <div role="tablist" aria-label="쇼케이스 보기 방식" className="flex gap-1 rounded-2xl bg-white/5 p-1 ring-1 ring-inset ring-white/10">
      {tabs.map((t) => {
        ...
        return (
          ...
          >
            <span className="block text-[1.2rem] sm:text-[1.417rem] font-black leading-tight">{t.label}</span>
            <span className={`mt-0.5 block text-[1.1rem] sm:text-[1.3rem] leading-tight ${on ? 'text-slate-700' : 'text-slate-500'}`}>{t.hint}</span>
          </button>
        )
      })}
    </div>
