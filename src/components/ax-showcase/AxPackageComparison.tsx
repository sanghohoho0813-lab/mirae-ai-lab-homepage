    ...
    <div>
      <div className="grid gap-3 lg:grid-cols-3">
        {AX_PACKAGES.map((p) => (
          ...
          >
            {p.recommended && p.badges && (
              <div className="mb-3 flex flex-wrap gap-1.5">
                {p.badges.map((b, i) => (
                  <span
                    key={b}
                    className={`break-keep rounded-full px-2.5 py-1 text-[1.0rem] font-black ${
            ...
            <p className="text-[1.0rem] font-black tracking-widest text-teal-300">{p.key}</p>
            <h3 className="mt-1 break-keep text-[1.5rem] font-black leading-tight text-white sm:text-[1.5rem]">{p.name}</h3>
            ...
            <p className="mt-3 flex items-baseline gap-2">
              ...
              <span className="text-[1.0rem] font-semibold text-slate-400">{p.priceNote}</span>
            </p>

            <p className="mt-3 break-keep text-[1.09rem] font-bold leading-relaxed text-slate-200">{p.oneLiner}</p>

            <ul className="mt-4 flex-1 space-y-1.5">
              {p.included.map((it) => (
                <li key={it} className="flex gap-2 break-keep text-[1.03rem] leading-snug text-slate-300">
            ...
            <p className="mt-4 break-keep rounded-xl bg-white/5 px-3 py-2.5 text-[1.0rem] leading-snug text-slate-300 ring-1 ring-inset ring-white/10">
      ...
      <ul className="mt-4 space-y-1">
        {AX_PACKAGE_NOTES.map((n) => (
          <li key={n} className="flex gap-2 break-keep text-[1.0rem] leading-relaxed text-slate-500">
