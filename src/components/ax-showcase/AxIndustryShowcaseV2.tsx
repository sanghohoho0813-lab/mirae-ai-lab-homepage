  ...
  return (
    <section id="ax-showcase-v2" className="relative scroll-mt-16 overflow-hidden border-t border-white/10 bg-slate-900">
      ...
      <div className="relative mx-auto max-w-5xl px-5 py-16 sm:px-6 sm:py-24">
        <h2 className="break-keep text-center text-[2.04rem] font-black leading-[1.4] tracking-[-0.015em] text-white sm:text-[3.38rem]">
          우리 업종이 AX로 바뀌면<br className="hidden sm:block" /> 무엇이 달라지는지 <span className="text-teal-300">직접 확인해보세요.</span>
        </h2>
        <p className="mx-auto mt-8 max-w-2xl break-keep text-center text-[1.32rem] leading-[1.75] text-slate-300 sm:text-[1.768rem]">
        ...
        <div className="mt-6">
          <div className="flex flex-wrap items-center gap-x-2.5 gap-y-1.5">
            {initialSlug && (
              <span className="rounded-md bg-amber-400/15 px-2.5 py-1 text-[1.13rem] sm:text-[1.339rem] font-black text-amber-200 ring-1 ring-inset ring-amber-400/30">
                선택한 업종 · {industry.displayName}
              </span>
            )}
            <span aria-hidden className="text-[1.65rem] sm:text-[1.95rem] leading-none">{industry.icon}</span>
            <h3 className="text-[1.45rem] font-black text-white sm:text-[1.95rem]">{industry.displayName}</h3>
            <span className="rounded-md bg-white/8 px-2 py-0.5 text-[1.1rem] sm:text-[1.3rem] font-bold text-slate-300 ring-1 ring-inset ring-white/15">
              화면 5장
            </span>
          </div>
          <p className="mt-2 max-w-3xl break-keep text-[1.2rem] leading-relaxed text-slate-300 sm:text-[1.495rem]">{industry.overview}</p>
          ...
          <div className="mt-4 flex flex-wrap gap-2.5">
            ...
            <Link
              ...
              className="inline-flex min-h-[48px] items-center gap-1.5 rounded-xl border border-teal-400/30 bg-teal-400/10 px-4 text-[1.2rem] sm:text-[1.417rem] font-bold text-teal-200 transition-colors hover:bg-teal-400/20 hover:text-teal-100"
            >
        ...
        <p className="mt-6 break-keep text-[1.1rem] sm:text-[1.3rem] leading-relaxed text-slate-500">{AX_V2_DISCLAIMER}</p>
      </div>
    </section>
  )
}
