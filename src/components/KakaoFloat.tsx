...
export default function KakaoFloat() {
  ...
  return (
    <a
      ...
      className={`fixed right-4 bottom-[calc(env(safe-area-inset-bottom,0px)+84px)] z-40 inline-flex items-center gap-2 rounded-full bg-[#FEE500] p-3.5 text-[1.17rem] sm:text-[1.06rem] font-black text-[#181600] shadow-lg shadow-slate-900/20 ring-1 ring-black/5 transition-all duration-200 hover:-translate-y-0.5 sm:right-6 sm:bottom-6 sm:px-4 sm:py-3 ${
        shown ? 'pointer-events-auto translate-y-0 opacity-100' : 'pointer-events-none translate-y-3 opacity-0'
      }`}
    >
      <svg viewBox="0 0 24 24" className="h-6 w-6 sm:h-5 sm:w-5" fill="currentColor" aria-hidden>
