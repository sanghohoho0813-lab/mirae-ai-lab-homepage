    ...
    <section className="relative overflow-hidden bg-slate-950">
      ...
      <div className="relative mx-auto flex min-h-[80svh] max-w-6xl flex-col justify-center px-5 py-7 sm:px-6 lg:grid lg:min-h-0 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.12fr)] lg:items-center lg:gap-10 lg:py-12">
        ...
        <div>
          ...
          <h1 className="mt-5 font-black tracking-tight text-[clamp(2.35rem,8vw,3rem)] leading-[1.15] sm:text-[clamp(2.7rem,4.8vw,3.3rem)] lg:text-[3rem]">
            <span className="block text-white">정책자금 신청에서 끝나지 않습니다.</span>
            <span className="mt-2.5 block text-teal-300">실제로 보이는 AX 화면까지 만듭니다.</span>
          </h1>
          <p className="mt-6 max-w-md text-[1.05rem] leading-relaxed text-slate-300 sm:text-[1.1rem]">
            자금 컨설턴트와 개발 담당자가 처음부터 같은 프로젝트로 참여합니다.
          </p>
          {/* CTA — AX 사례 우선, 진단은 보조 */}
          <div className="mt-8 flex flex-col gap-3 sm:max-w-md sm:flex-row">
            <button type="button" onClick={onShowcase} className="shine-cta flex items-center justify-center gap-2 rounded-xl bg-teal-400 px-6 py-4 text-base font-black text-slate-900 shadow-lg shadow-teal-500/20 transition-transform hover:-translate-y-0.5 hover:bg-teal-300">
              AX 실행 사례 보기 <span aria-hidden>↓</span>
            </button>
            <Link to="/business-diagnosis" className="flex items-center justify-center gap-2 rounded-xl border border-white/30 bg-white/5 px-6 py-4 text-base font-bold text-white transition-colors hover:bg-white/10">
              <span aria-hidden>🩺</span> 3분 기업진단
            </Link>
          </div>
          <button type="button" onClick={onProgram} className="mt-4 inline-flex items-center gap-1 text-[0.9rem] font-semibold text-teal-200/90 underline underline-offset-4 transition-colors hover:text-teal-100">
