        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {AX_DIFF_COLS.map((c) => (
            <div key={c.key} className={`flex flex-col rounded-3xl border p-6 ${c.highlight ? 'border-2 border-blue-500 bg-white shadow-lg shadow-blue-500/10' : 'border-slate-200 bg-slate-50'}`}>
              <p className={`text-[0.98rem] font-black ${c.highlight ? 'text-blue-600' : 'text-slate-500'}`}>{c.label}</p>
              <p className={`mt-0.5 text-[0.8rem] font-semibold ${c.highlight ? 'text-slate-500' : 'text-slate-400'}`}>{c.sub}</p>
              <ul className="mt-4 space-y-2.5">
                {c.items.map((it) => (
                  <li key={it} className={`flex items-start gap-2.5 text-[0.92rem] leading-snug ${c.highlight ? 'font-semibold text-slate-700' : 'text-slate-500'}`}>
                    {c.highlight
        ...
        <div className="mt-6 rounded-3xl border border-slate-200 bg-slate-50 p-5 sm:p-6">
          <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[1.02rem] font-black text-slate-900">미래 AI 랩 대표 · 미래경영지원센터</p>
