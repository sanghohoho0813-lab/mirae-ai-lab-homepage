    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <header className="sticky top-0 z-30 border-b border-white/10 bg-slate-950/90 backdrop-blur">
        <div className="mx-auto flex max-w-[880px] items-center justify-between gap-3 px-5 py-3">
          <Link to="/business-services" className="flex items-center gap-1.5 text-[0.9rem] font-bold text-slate-300 transition-colors hover:text-white">
      ...
      <section className={`${band} border-b border-white/10 bg-slate-900`}>
        <div className={inner}>
          <p className="flex items-center gap-2 text-[0.82rem] font-black uppercase tracking-widest text-teal-300">
            <span aria-hidden className="text-[1.15rem] leading-none">{industry.icon}</span> {industry.displayName} AX 사업화
          </p>
          <h1 className="mt-3 break-keep text-[1.75rem] font-black leading-[1.28] text-white sm:text-[2.5rem]">
            {industry.shortHook}
          </h1>
          <p className="mt-4 max-w-2xl break-keep text-[1rem] leading-relaxed text-slate-300 sm:text-[1.1rem]">{industry.overview}</p>
          <div className="mt-6 flex flex-col gap-2.5 sm:max-w-lg sm:flex-row">
            <Link to="/business-diagnosis" className="flex min-h-[52px] items-center justify-center gap-2 rounded-xl bg-blue-500 px-5 text-[1rem] font-black text-white transition-transform hover:-translate-y-0.5 hover:bg-blue-400">
              <span aria-hidden>🩺</span> 3분 기업진단
            </Link>
            <Link to="/business-services#ax-showcase-v2" className="flex min-h-[52px] items-center justify-center rounded-xl border border-white/25 bg-white/5 px-5 text-[1rem] font-bold text-white transition-colors hover:bg-white/10">
              다른 업종 보기
            </Link>
          </div>
        </div>
        ...
        <div className={inner}>
          ...
          <div className="mt-4 grid gap-2.5 sm:grid-cols-2">
            {industry.ideas.map((idea) => (
              <p key={idea.no} className="break-keep rounded-2xl border border-white/12 bg-white/[0.04] p-4 text-[0.95rem] leading-relaxed text-slate-300">
                {idea.problem}
              </p>
            ))}
          </div>
          <p className="mt-4 break-keep border-l-2 border-amber-400 pl-3.5 text-[1rem] font-bold leading-relaxed text-amber-100 sm:text-[1.08rem]">
      ...
      <section className={`${band} border-b border-white/10 bg-slate-900`}>
        <div className={inner}>
          ...
          <p className="mt-2.5 max-w-2xl break-keep text-[0.95rem] leading-relaxed text-slate-400">
      ...
      <section className={`${band} border-b border-white/10 bg-slate-900`}>
        <div className={inner}>
          ...
          <div className="mt-4 grid gap-3 lg:grid-cols-2">
            {industry.ideas.map((idea) => (
              <AxBusinessIdeaCard key={idea.no} idea={idea} industryName={industry.displayName} variant="full" />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className={`${band} border-b border-white/10 bg-slate-950`}>
        <div className={inner}>
          ...
          <p className="mt-2.5 break-keep text-[0.92rem] leading-relaxed text-slate-400">
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {industry.ideas.flatMap((i) => i.revenues).map((r, i) => (
              <div key={`${r}-${i}`} className="flex items-start gap-2 rounded-xl border border-amber-400/20 bg-amber-400/[0.07] p-3">
                <span aria-hidden className="mt-0.5 text-[0.9rem]">💰</span>
                <span className="break-keep text-[0.9rem] font-bold leading-snug text-amber-100">{r}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className={`${band} border-b border-white/10 bg-slate-900`}>
        <div className={inner}>
          ...
          <p className="mt-3 break-keep rounded-2xl border border-teal-400/25 bg-teal-400/[0.08] p-4 text-[0.98rem] font-bold leading-relaxed text-teal-100 sm:text-[1.05rem]">
          ...
          <ul className="mt-4 space-y-2">
            {[
              ...
              <li key={t} className="flex gap-2 break-keep text-[0.95rem] leading-relaxed text-slate-300">
                <span aria-hidden className="mt-[0.45rem] h-1.5 w-1.5 shrink-0 rounded-full bg-teal-400" />
                {t}
              </li>
            ))}
          </ul>
          <p className="mt-3 break-keep text-[0.85rem] leading-relaxed text-slate-500">
      ...
      <section className={`${band} border-b border-white/10 bg-slate-950`}>
        <div className={inner}>
          ...
          <div className="mt-4 rounded-2xl border border-amber-400/40 bg-amber-400/[0.08] p-4 sm:p-5">
            <p className="text-[0.8rem] font-black tracking-widest text-amber-300">가장 추천</p>
            <p className="mt-1 text-[1.2rem] font-black text-white sm:text-[1.35rem]">{recommended.name}</p>
            <p className="mt-2 break-keep text-[0.95rem] leading-relaxed text-slate-200">{recommended.oneLiner}</p>
            <p className="mt-2.5 break-keep text-[0.88rem] leading-relaxed text-slate-400">{recommended.fit}</p>
          </div>
          <Link
            to="/business-services/funding-consulting#ax-packages"
            className="mt-3.5 inline-flex min-h-[48px] items-center gap-1.5 rounded-xl border border-white/25 bg-white/5 px-4 text-[0.95rem] font-bold text-white transition-colors hover:bg-white/10"
          >
          ...
          <div className="mt-4 space-y-2">
            {[
              ...
              <details key={f.q} className="group rounded-2xl border border-white/12 bg-white/[0.04] p-4">
                <summary className="cursor-pointer list-none break-keep text-[0.98rem] font-black text-white marker:hidden">
                  <span className="mr-2 text-teal-300" aria-hidden>Q</span>
                  {f.q}
                </summary>
                <p className="mt-2.5 break-keep text-[0.92rem] leading-relaxed text-slate-300">{f.a}</p>
              </details>
            ))}
          </div>
        </div>
      ...
      <section className={`${band} border-b border-white/10 bg-slate-950`}>
        <div className={inner}>
          <h2 className="text-[1.1rem] font-black text-white sm:text-[1.25rem]">다른 업종도 확인해보세요</h2>
          <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">
            {others.map((o) => (
              <Link
                key={o.slug}
                to={`/ax-industries/${o.slug}`}
                className="flex min-h-[52px] items-center gap-2 rounded-xl border border-white/12 bg-white/[0.04] px-3 py-2 text-[0.9rem] font-bold text-slate-200 transition-colors hover:border-teal-400/40 hover:text-white"
              >
                <span aria-hidden className="text-[1.05rem] leading-none">{o.icon}</span>
      ...
      <section className={`${band} bg-slate-900`}>
        <div className={inner}>
          ...
          <Link
            ...
            className="mt-5 inline-flex min-h-[56px] items-center gap-2 rounded-xl bg-blue-500 px-6 text-[1.05rem] font-black text-white transition-transform hover:-translate-y-0.5 hover:bg-blue-400"
          >
            <span aria-hidden>🩺</span> 3분 기업진단 시작하기
          </Link>
          <p className="mt-5 break-keep text-[0.82rem] leading-relaxed text-slate-500">{AX_V2_DISCLAIMER}</p>
        </div>
      </section>
