...
export const AX_PACKAGES: AxPackage[] = [
  {
    ...
    price: '100만원',
    priceNote: 'VAT 별도',
    oneLiner: '우리 회사가 어떤 AX 사업이 될 수 있는지 먼저 설계해요.',
    included: [
      '대표자·기업 인터뷰',
      '현재 사업과 자금문제 진단',
      'AX로 바꿀 핵심업무 선정',
  ...
  },
  {
    ...
    price: '300만원',
    priceNote: 'VAT 별도',
    oneLiner: 'A에 벤처·연구소 준비를 더해, 혁신기업이라는 근거를 만듭니다.',
    included: [
      'A 프로그램 전체',
      '벤처기업 혁신성장유형 준비',
    ...
    fit: '1억원 이상 정책자금과 성장을 본격 준비하는 회사',
    recommended: true,
  ...
  },
  {
    ...
    price: '500만원',
    priceNote: 'VAT 별도',
    oneLiner: 'B에 특허와 다음 자금 로드맵까지 더해, 장기 성장을 준비해요.',
    included: [
      'B 프로그램 전체',
      '특허 아이디어 발굴과 기술구조 설계',
    ...
    fit: '기술자산과 다음 자금까지 한 번에 준비하려는 회사',
    recommended: false,
...
export const AX_PACKAGE_NOTES: string[] = [
  '전 금액 VAT 별도입니다.',
  '특허는 제휴 변리사가, 세무·노무·법률은 해당 자격 전문가가 맡습니다.',
  '벤처·연구소·특허의 결과와 처리기간은 외부기관 심사·절차에 따라 달라집니다.',
  '세 프로그램은 AX 사업화·컨설팅·인증 준비 패키지이며, 운영형 본개발은 별도 계약입니다.',
]
...
export const AX_BUILD_STAGES: AxBuildStage[] = [
  {
    ...
    name: '시연형 화면 완성',
    price: '500만원',
    level: '심사에서 직접 눌러 보여 주는 수준',
    duration: '착수일로부터 약 1~7일 소요',
    tone: 'from-slate-700 to-slate-500',
    toneText: 'text-white',
    items: [
      '핵심 화면 8~12개를 눌러 볼 수 있게 만듭니다.',
      '대표·직원·고객 화면의 흐름을 잇습니다.',
      '로그인·데이터 저장은 포함하지 않는 초기 모델이에요.',
      '심사 자료와 같은 구조로 정리해 드립니다.',
    ],
  },
  {
    no: '2',
    ...
    levelTag: '안정권 수준',
    items: [
      '로그인, 사용자 권한, 데이터 저장이 들어가요.',
      '핵심 업무 한 가지가 실제로 돌아갑니다.',
      '관리자 화면과 현장용 모바일 화면을 함께 드려요.',
      '실사용 테스트와 초기 개선까지 포함합니다.',
    ],
  },
  {
    no: '3',
    ...
    best: true,
    bestNote: '정책자금·정부지원사업에서 설명하기 가장 알맞은 구성이에요.',
    items: [
...
/** 개발 3단계 표 아래 고지 — 일정이 확정 약속으로 읽히지 않게 한다. */
export const AX_BUILD_STAGE_NOTE =
  '일정은 자료 접수와 결정이 원활할 때의 목표이며, 외부 연동·데이터 이전 범위·추가 요청에 따라 달라질 수 있습니다. 금액은 모두 VAT 별도입니다.'

/** 후불 정산 구조 설명 — "개발이 공짜"로 오해되지 않게 한다. */
export const AX_BUILD_PAYMENT = {
  title: '개발비를 수천만원 먼저 내야 하나요?',
  lines: [
    '아니요. 컨설팅 비용만 먼저 내시면 개발을 시작해요.',
    '개발비가 무료라는 뜻은 아닙니다. 정책자금이 조달된 뒤에 정산합니다.',
  ],
  notes: [
    '정산 금액과 시점은 조달금액과 실제 구현범위에 따라 계약서로 확정합니다.',
...
export const AX_METHOD_STEPS: AxMethodStep[] = [
  { no: 1, icon: '🔍', title: '숨은 사업자산 발견', desc: '대표님의 경험과 고객, 반복 업무에서 기술·데이터가 될 부분을 찾아요.' },
  { no: 2, icon: '🔄', title: '현재 업무 AX 전환', desc: '엑셀, 카톡, 수기로 하던 일을 관리자와 직원이 함께 쓰는 한 흐름으로 모읍니다.' },
  { no: 3, icon: '🚀', title: '새 사업모델로 확장', desc: '내부 관리에서 끝내지 않고, 고객용 앱·웹과 반복매출 모델까지 넓혀요.' },
  { no: 4, icon: '🏅', title: '기술 근거 만들기', desc: '필요하면 벤처·연구소·특허로 혁신성과 개발계획의 근거를 만듭니다.' },
  { no: 5, icon: '💰', title: '자금과 성장 연결', desc: '사업계획·자금사용계획을 실제 화면과 맞추고, 다음 기관과 순서까지 짜요.' },
]

/** 미래AI랩이 만드는 세 가지 가치 */
export const AX_CORE_VALUES = [
  { icon: '🏛️', title: '정책자금 심사에 맞춘 사업구조', desc: '기술성·사업성·성장성이 보이게 회사를 다시 정리해요.' },
  { icon: '🖥️', title: '눈으로 보이는 실행 증거', desc: '문서 속 계획을 실제 화면과 업무 흐름으로 보여 줍니다.' },
  { icon: '📈', title: '자금 이후 성장 로드맵', desc: '매출로 이어질 다음 순서까지 함께 짜요.' },
]
