...
export const businessPackages: BusinessPackage[] = [
  ...
  },
  {
    ...
    tagline: '매일 반복되는 업무 1가지를 자동화 프로그램으로 만들어, 시간과 비용을 대폭 절감해 드립니다.',
    ...
    deliverables: ['반복업무 자동화 프로그램 1가지', '업무 진단 및 자동화 설계', 'PC·모바일 대응', '풀패키지 AX 확장 방향 안내'],
    process: '업무 진단 → 자동화 설계 → 프로그램 구축 → 사용법 안내',
    expectation: '매일 반복하던 업무 1가지가 자동화되어, 거기에 들어가던 시간과 인건비가 줄어듭니다.',
    why: '반복업무를 사람이 계속 처리하면 시간과 실수가 늘어납니다. 가장 자주 반복되는 업무 1가지부터 자동화 프로그램으로 만들면, 적은 비용으로 AX의 효과를 바로 체감할 수 있습니다.',
    faqs: [
      { q: '어떤 업무를 자동화할 수 있나요?', a: '반복 입력·조회·리포트 등 대표님 업무를 진단해 자동화 효과가 가장 큰 업무부터 함께 정합니다.' },
      { q: '풀패키지 AX 시스템 구축과는 무엇이 다른가요?', a: '이 상품은 반복업무 1가지를 자동화하는 일회성 기초 모델입니다. 여러 업무를 통합 자동화하고 대시보드·운영 관리까지 포함하는 풀패키지 구축은 세팅 비용과 월 운영 비용이 발생해, 상담 후 범위와 견적을 정합니다.' },
      { q: '모바일에서도 쓸 수 있나요?', a: 'PC·모바일 모두 대응되는 구조로 만들어 어디서든 확인할 수 있습니다.' },
    ],
    visualType: 'mvp',
...
export const packageBanner: Record<string, { title: string; subtitle: string; accent: BannerAccent }> = {
  ...
  'ai-ax-system': { title: 'AI 업무 자동화 프로그램', subtitle: '반복업무 1가지 자동화 · 일회성 구축', accent: 'cyan' },
