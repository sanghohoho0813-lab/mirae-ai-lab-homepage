...
export const CATEGORY_SCENARIOS: Record<string, string> = {
  ...
  'AX 컨설팅': '회사 일을 디지털로 정리하고 싶다면',
  '풀패키지': '성장 준비를 한 곳에 맡기고 싶다면',
  '법인 컨설팅': '법인에 쌓인 돈을 계획적으로 정리하고 싶다면',
}

// 카테고리별 보조 안내 — 섹션 제목 아래 노출 (전문성 신뢰 확보용)
export const CATEGORY_NOTES: Record<string, string> = {
  '법인 컨설팅': '가지급금, 이익잉여금, 가업승계 같은 세무 일은 세무사 등 전문 자격사와 함께 진행합니다.',
}
...
export const businessPackages: BusinessPackage[] = [
  {
    // 1층 대표 프로그램 — 50만원 단품 컨설팅을 A·B·C(100/300/500만원) 프로그램으로 대체했다.
    // 상세는 /business-services/funding-consulting (AX 사업화·자금조달 상세페이지)가 담당한다.
    ...
    badge: '정책자금 × AX',
    badgeTone: 'blue',
    name: '정책자금 × AX 사업화 프로그램',
    tagline: '자금조달 전략과 업종 맞춤 AX 실행설계, 실제로 보여줄 수 있는 화면까지 하나의 프로젝트로 진행합니다.',
    short: '자금전략 + AX 실행설계 + 보여줄 수 있는 화면까지',
    price: '100만원부터',
    priceType: 'consult',
    priceNote: 'A 100만원 · B 300만원 · C 500만원 (VAT 별도) — 진단 후 단계를 정합니다',
    priceHighlight: '본개발비는 자금조달 이후 정산',
    highlights: [
      'A·B·C 세 단계 — 컨설팅비 100만원부터 시작',
      '자금 컨설턴트와 개발자가 처음부터 함께 참여',
      '최대 2주 안에 결과물 완성 목표',
    ],
    highlightNote: '사업계획서에서 끝나지 않고 실제 화면까지 만듭니다',
    recommendedFor: [
      '사업계획서만으로는 차이를 보여주기 어려웠던 대표님',
      '1억원 이상 성장자금이 실제로 필요한 대표님',
      '자금조달과 함께 업무방식까지 바꾸려는 대표님',
    ],
    deliverables: [
      '자금조달 전략 — 어떤 기관에 어떤 순서로 신청할지',
      '사업계획과 자금사용계획',
      '업종 맞춤 AX 업무 흐름 정리',
      '실제로 열어서 보여줄 수 있는 화면(선택한 단계 기준)',
      '자금 이후 성장 로드맵',
    ],
    process: '기업진단 → 자금전략과 AX 설계 → 화면 제작 → 신청 준비',
    expectation:
      '검토할 자금 방향이 정리되고, 사업계획과 실제 화면이 같은 내용을 말하도록 연결됩니다.',
    why: '요즘은 AI로 누구나 보기 좋은 사업계획서를 만들 수 있어 문장만으로 차이를 보여주기 어렵습니다. 지금 하는 업무를 AX로 바꾸면, 문서가 아니라 실제 화면과 데이터로 설명할 수 있습니다.',
    faqs: [
      { q: '정책자금 승인을 보장하나요?', a: '아니요. 승인·금리·한도는 기관 심사 사항입니다. 저희는 가능성 진단과 신청 전략, 그리고 심사에서 설명할 실행근거를 함께 만듭니다.' },
      { q: '100만원과 300만원, 500만원은 어떻게 다른가요?', a: 'A(100만원)는 자금전략과 AX 실행설계, B(300만원)는 여기에 시연형 화면 제작까지, C(500만원)는 업무에 쓰는 수준의 구성까지 포함합니다. 진단과 상담 후 필요한 단계를 함께 정합니다.' },
      { q: '개발비를 먼저 수천만원 내야 하나요?', a: '아니요. 컨설팅 비용만 먼저 납부하시면 개발을 시작합니다. 개발비는 무료가 아니며, 정책자금이 조달된 이후에 정산하는 구조입니다.' },
      { q: '업종을 바꿔야 하나요?', a: '아닙니다. 업종을 바꾸는 것이 아니라 지금 하고 있는 업무방식을 AX로 바꿉니다.' },
  ...
  },
  {
    ...
    priceType: 'consult',
    highlights: ['반복업무 자동화 — 시간·인건비 절감', '업무를 먼저 진단하고, 꼭 필요한 만큼만 제작', 'PC·모바일 어디서나 사용'],
  ...
  },
  {
    ...
    priceType: 'consult',
    highlights: ['출연금 손금 인정 — 법인세 부담 절감', '직원 복지 혜택, 소득세·4대보험 부담 없이(요건 충족 시)', '오래 다니고 싶게 만드는 복지'],
  ...
  },
  {
    ...
    priceType: 'consult',
    highlights: [
      '자금·인증·홈페이지·AI를 한 번에 설계',
      '업무 자동화 시스템(HR·CRM·ERP·회의 기록) — 회사를 스마트폰에서 한눈에',
      '법인 자금·절세 구조 정리(가지급금·이익잉여금 등) — 세금은 줄이고, 자금은 합법적으로 대표님 자산으로',
