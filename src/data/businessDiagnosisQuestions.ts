...
// v2: 인증 질문을 개별 보유상태 확인으로 재구성 (지식재산권·연구조직·벤처·이노비즈·메인비즈·ISO·연구개발비·신제품·영업이익)
...
export const questions: DiagnosisQuestion[] = [
  ...
  },
  {
    ...
    stage: 3,
    type: 'single',
    title: '최근 3년 내 등록된 지식재산권이 있으신가요?',
    desc: '출원만 한 것과 등록이 완료된 것은 구분해주세요.',
    // 예비창업자는 기존 기업 대상 인증 질문 생략 (창업 후 검토 안내)
    showIf: (a) => !isFounderToBe(a),
    options: [
      { value: 'patent', label: '등록 특허 보유' },
      { value: 'utility', label: '등록 실용신안·디자인 보유' },
      { value: 'filed', label: '출원만 진행', desc: '아직 등록 전이에요' },
  ...
  },
  {
    ...
    showIf: (a) => !isFounderToBe(a),
    options: [
      { value: 'lab', label: '기업부설연구소 보유' },
      { value: 'dept', label: '연구개발전담부서 보유' },
      { value: 'preparing', label: '설립 준비 중' },
      { value: 'none', label: '없음' },
      { value: 'unsure', label: '잘 모르겠음' },
    ],
  },
  ...
  },
  {
    ...
    // 통상 업력 3년 이상 대상 — 조건 미충족 시 생략
    showIf: (a) => !isFounderToBe(a) && ['y3to7', 'y7plus'].includes(one(a, 'years') ?? ''),
    options: [
      { value: 'have', label: '보유' },
      { value: 'preparing', label: '준비 중' },
      { value: 'expired', label: '과거 보유했으나 만료' },
  ...
  },
  {
    ...
    stage: 3,
    type: 'multi',
    title: 'ISO 인증을 보유하고 계신가요?',
    desc: '해당하는 것을 모두 골라주세요.',
    showIf: (a) => !isFounderToBe(a),
    exclusiveValues: ['none', 'unsure'],
    options: [
      { value: 'iso9001', label: 'ISO9001 (품질) 보유' },
      { value: 'iso14001', label: 'ISO14001 (환경) 보유' },
      { value: 'iso45001', label: 'ISO45001 (안전보건) 보유' },
      { value: 'isoEtc', label: '기타 ISO 보유' },
      { value: 'preparing', label: '준비 중' },
      { value: 'none', label: '없음' },
      { value: 'unsure', label: '잘 모르겠음' },
    ],
  },
  {
    ...
    stage: 3,
    type: 'single',
    title: '최근 결산 기준으로 연구개발비가 매출의 약 5% 이상인지 알고 계신가요?',
    // '잘 모르겠음'은 불리하게 점수화하지 않고 '재무자료 추가 확인 필요'로 분류
    showIf: (a) => !isFounderToBe(a) && one(a, 'revenue') !== 'none',
    options: [
      { value: 'ge5', label: '5% 이상' },
      { value: 'lt5', label: '5% 미만' },
      { value: 'notManaged', label: '연구개발비를 별도로 관리하지 않음' },
      { value: 'unsure', label: '잘 모르겠음' },
    ],
  },
  {
    ...
    stage: 3,
    type: 'single',
    title: '최근 신제품·신서비스 개발 실적이 있으신가요?',
    showIf: (a) => !isFounderToBe(a),
    options: [
      { value: 'launched3y', label: '최근 3년 내 출시 또는 양산 실적 있음' },
      { value: 'developing', label: '현재 개발 중' },
      { value: 'planned', label: '계획만 있음' },
      { value: 'none', label: '없음' },
      { value: 'na', label: '해당 없음' },
    ],
  },
  {
    ...
    stage: 3,
    type: 'single',
    title: '최근 결산에서 영업이익 상황은 어땠나요?',
    showIf: (a) => !isFounderToBe(a),
    options: [
      { value: 'profit', label: '영업이익 발생' },
      { value: 'breakeven', label: '손익분기 수준' },
      { value: 'loss', label: '영업손실' },
      { value: 'notClosed', label: '아직 결산 전' },
      { value: 'unsure', label: '잘 모르겠음' },
    ],
  },
  {
