...
export const productRules: ProductRule[] = [
  {
    ...
    reason: (a) =>
      arrears(a)
        ? '자금 수요는 있지만 체납이라는 먼저 해결할 과제가 있어, 정리 이후 가능성 진단부터 차분히 검토하는 것이 좋습니다.'
        : `자금 필요 시점이 ${
            one(a, 'fundingWhen') === 'm1' ? '1개월 이내' : one(a, 'fundingWhen') === 'm3' ? '3개월 이내' : one(a, 'fundingWhen') === 'm6' ? '6개월 이내' : '가까워'
          }이고, 심사자에게 보여줄 자료를 함께 준비하면 검토가 수월해집니다.`,
