// /api/trial/start — 자체 포함(import 0개). 모듈 로드 실패가 불가능하도록 작성.
// - GET: health 체크 (브라우저 주소창으로 확인 가능)
// - POST: 7일 체험 시작. Supabase 는 핸들러 안에서 동적 import 만 사용.
// 외부 helper(api/_supabase, api/_trial) 를 import 하지 않습니다.
...
export default async function handler(req: any, res: any) {
  try {
    ...
    console.log('[trial/start] POST received')
    ...
    // 7) profile 확인 (tool_access.user_id FK 대상)
    const { data: profile, error: profErr } = await admin.from('profiles').select('id').eq('id', user.id).maybeSingle()
    if (profErr) return res.status(500).json({ ok: false, message: '프로필 조회에 실패했습니다.', debugCode: 'no_profile', detail: detailOf(profErr) })
    if (!profile) return res.status(400).json({ ok: false, message: '프로필이 없습니다. 다시 로그인하거나 관리자에게 문의해 주세요.', debugCode: 'no_profile' })
    ...
    if (upErr) return res.status(500).json({ ok: false, message: '체험 시작 저장에 실패했습니다.', debugCode: 'access_upsert', detail: detailOf(upErr) })
