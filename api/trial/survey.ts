...
import { EXTENSION_DAYS, MAX_FREE_DAYS, computeTrialExpiry } from '../_trial'
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const { error: insErr } = await admin
      .from('surveys')
      .insert({ user_id: user.id, tool_id: toolId, answers: answers ?? {} })
    if (insErr) return fail(res, 500, '설문 저장에 실패했습니다.', 'survey_insert', insErr)
    ...
    if (rec.survey_extension_used) return fail(res, 409, '설문 연장은 1회만 가능합니다.', 'survey_used')
